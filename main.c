...
int main(int argc, char *argv[]) { //示例 mpirun -np 4 ./laplace_solver 4096
    ...
    for (iter = 0; iter < MAX_ITER; iter++) {
        ...
        // 上下方向通信
        if (coords[0] != 0) {
            MPI_Irecv(&u[0 * (local_Ny + 2)], local_Ny + 2, MPI_DOUBLE, up, 0, cart_comm, &requests[request_count++]); // 接收上方
            MPI_Isend(&u[1 * (local_Ny + 2)], local_Ny + 2, MPI_FLOAT, up, 0, cart_comm, &requests[request_count++]);  // 发送到下方
        } 
        else {
        ...
        if (coords[0] != dims[0] - 1) {
            MPI_Irecv(&u[(local_Nx + 1) * (local_Ny + 2)], local_Ny + 2, MPI_FLOAT, down, 0, cart_comm, &requests[request_count++]); // 接收下方
            MPI_Isend(&u[local_Nx * (local_Ny + 2)], local_Ny + 2, MPI_FLOAT, down, 0, cart_comm, &requests[request_count++]); // 发送到上方
        } 
        else {
        ...
        if (coords[1] != 0) {
            for (int i = 0; i < local_Nx + 2; i++) {
                MPI_Irecv(&u[i * (local_Ny + 2)], 1, MPI_FLOAT, left, 0, cart_comm, &requests[request_count++]);   // 接收左侧
                MPI_Isend(&u[i * (local_Ny + 2) + 1], 1, MPI_FLOAT, left, 1, cart_comm, &requests[request_count++]);   // 发送到左侧，tag为1
            }
        } 
        else {
        ...
        if (coords[1] != dims[1] - 1) {
            for (int i = 0; i < local_Nx + 2; i++) {
                MPI_Irecv(&u[i * (local_Ny + 2) + local_Ny + 1], 1, MPI_FLOAT, right, 1, cart_comm, &requests[request_count++]); // 接收右侧，tag为1
                MPI_Isend(&u[i * (local_Ny + 2) + local_Ny], 1, MPI_FLOAT, right, 0, cart_comm, &requests[request_count++]);     // 发送到右侧
            }
        } 
        else {
        ...
        if (global_max_diff < TOL) break; // 满足条件，跳出迭代
    }
