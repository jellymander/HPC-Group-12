...
__global__ void jacobi_update(float *u, float *new_u, int Nx, int Ny, float *blk_diff) { 
...
extern "C" void cuda_jacobi(float *u, float *new_u, int local_Nx, int local_Ny, float *local_diff) {
    ...
    jacobi_update<<<grid, block>>>(d_u, d_new_u, local_Nx, local_Ny, d_local_diff); // Jacobi 迭代，执行 kernel
