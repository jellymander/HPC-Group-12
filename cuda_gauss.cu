...
__global__ void gauss_seidel_update(float *u, int Nx, int Ny,int gridx, int gridy, int restx, int resty, float *blk_diff, int flag) {
	...
	//printf("%d,%d,%d,%d,%d,%d\n",blockDim.x, blockDim.y,blockIdx.x, blockIdx.y,i,j);
    ...
    if ((i + j) % 2 == flag) {
        ...
        //); // 红点更新
		atomicExch(&u[i * (Ny + 2) + j],0.25 * (u[(i - 1) * (Ny + 2) + j] + u[(i + 1) * (Ny + 2) + j] +u[i * (Ny + 2) + (j - 1)] + u[i * (Ny + 2) + (j + 1)]));
    }
...
extern "C" void cuda_gauss_seidel(float *u, float *new_u, int local_Nx, int local_Ny, float *local_diff) {
    ...
    int flag = 0;
    gauss_seidel_update<<<grid, block>>>(d_u, local_Nx, local_Ny,gridx, gridy, restx, resty, d_local_diff, flag); // 红黑点 Gauss-Seidel 迭代，执行 kernel
	//printf("here3\n");
    flag = 1;
    gauss_seidel_update<<<grid, block>>>(d_u, local_Nx, local_Ny,gridx, gridy, restx, resty, d_local_diff, flag); // 红黑点 Gauss-Seidel 迭代，执行 kernel
